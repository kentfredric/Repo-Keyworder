...
# Identifies all perl packages that have stable keywords whatsoever
# and then reports what is missing with regards to stable keywords on the latest version
...
sub keyword_check {
  ...
  my $missing = $r->get_ebuild_missing_keywords_stable( "${category}/${package}/${package}-${best}.ebuild", $keywords );
